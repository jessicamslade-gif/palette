...
export async function buildCatalogPdf(project, links) {
  ...
  // Branded header band (dark background so the white logo mark shows up).
  try {
    ...
    const scale = logoSize / logoImg.height;
    ...
    // logo missing or unreadable — fall back to no header band
  }
